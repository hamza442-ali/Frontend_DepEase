...
const [projectOptions, setProjectOptions] = useState([])
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/project/getall')
      .then((response) => {
  ...
  const sendSlotsToBackend = () => {
    
    console.log(slots, " Slot Data ")
    axios.post('http://localhost:3001/scheduling/create', { slots })
      .then((response) => {
        console.log('Slots sent to the backend:', response.data);
        // Optionally, you can perform additional actions after a successful response
      })
      .catch((error) => {
        console.error('Error sending slots to the backend:', error);
  ...
  const addSlot = () => {
    if (newSlot && type && span > 0 && newSlot.trim() !== '' && type.trim() !== '') {
      ...
      const day = slotStartTime.toLocaleDateString(undefined, { weekday: 'long' });


      setSlots([...slots, { date: selectedDate, start: slotStartTime, end: slotEndTime, content: newSlot, type: type, day: day }]);
      setNewSlot('');
      setType('');
    }
  };
